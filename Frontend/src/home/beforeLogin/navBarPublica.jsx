...
const NavbarPublica = () => {
    ...
    // Función para saber si la ruta actual coincide con el botón
    const isActive = (path) => location.pathname === path;

    return (
        <nav className="tw-w-full tw-bg-primario-900 tw-grid tw-grid-cols-3 tw-items-center tw-shadow-sm tw-sticky tw-top-0 tw-z-50">
            {/* Logo Pequeño */}
            <div className="tw-h-16 tw-flex tw-items-center tw-px-6 tw-border-b tw-border-gray-800 tw-bg-primario-900">
                <div onClick={() => navigate('/')} className="tw-flex tw-items-center tw-gap-2 tw-cursor-pointer">
...
-102 436 -33 65 -33 65 -36 245 -4 203 -8 215 -72 215 -25 0 -43 -7 -56 -21z" />
                ...
                <p onClick={() => navigate('/')} className="tw-text-white tw-text-lg tw-font-bold tw-mb-2 tw-pl-3 tw-pt-1 tw-cursor-pointer">FoodStocker</p>
            </div>

            {/* Enlaces de Navegación - Centrados */}
            <div className="tw-hidden md:tw-flex tw-items-center tw-justify-center tw-gap-8">
                <button
                    onClick={() => navigate('/quienes-somos')}
                    className={`tw-flex tw-items-center tw-gap-2 tw-text-sm tw-font-medium tw-transition-colors tw-bg-transparent tw-border-none tw-cursor-pointer ${isActive('/quienes-somos') ? 'tw-text-secundario-400' : 'tw-text-white hover:tw-text-secundario-200'}`}
                >
                    <Users className="tw-w-4 tw-h-4" /> ¿Quiénes Somos?
                </button>
                <button
                    onClick={() => navigate('/contacto')}
                    className={`tw-flex tw-items-center tw-gap-2 tw-text-sm tw-font-medium tw-transition-colors tw-bg-transparent tw-border-none tw-cursor-pointer ${isActive('/contacto') ? 'tw-text-secundario-400' : 'tw-text-white hover:tw-text-secundario-200'}`}
                >
                    <Contact className="tw-w-4 tw-h-4" /> Contacto
                </button>
                <button
                    onClick={() => navigate('/documentos')}
                    className={`tw-flex tw-items-center tw-gap-2 tw-text-sm tw-font-medium tw-transition-colors tw-bg-transparent tw-border-none tw-cursor-pointer ${isActive('/documentos') ? 'tw-text-secundario-400' : 'tw-text-white hover:tw-text-secundario-200'}`}
                >
                    <FileText className="tw-w-4 tw-h-4" /> Documentos
                </button>
            </div>

            {/* Botón Iniciar Sesión - Alineado a la derecha */}
            <div className="tw-flex tw-justify-end tw-pr-6">
                <button
                    
                    onClick={() => navigate('/login')}
                    className="tw-border-none tw-flex tw-items-center tw-gap-3 tw-px-3 tw-py-2 tw-rounded-lg tw-text-sm tw-font-medium tw-transition-all tw-duration-150 tw-ease-in-out tw-bg-white tw-text-primario-950 hover:tw-bg-secundario-200">
                    Ingresar <LogIn className="tw-w-4 tw-h-4" /> 
                </button>
            </div>
        </nav>
    );
