...
export const cargarVencidos = async (req, res) => {
    try {
        const idResponsable = req.body?.Id_Responsable || req.user?.id;
        const creadas = await PerdidasService.cargarVencidosAutomaticos(idResponsable);
        res.status(200).json({
            mensaje: `Se han registrado automáticamente ${creadas.length} pérdidas de insumos vencidos`,
            total: creadas.length,
            data: creadas
        });
    } catch (error) {
        res.status(500).json({
            mensaje: 'Error al cargar los insumos vencidos',
            error: error.message
        });
    }
};
